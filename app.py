...
st.set_page_config(page_title="Klasifikasi KBLI 2 Digit – Versi Akurasi Tinggi", layout="wide")
st.title("Klasifikasi KBLI 2 Digit (Akurasi Tinggi + Regex Aman)")

st.write("""
Upload file mentah (CSV/Excel) berisi minimal:
- r215a1_label / r215b / r215d (teks)
- r216_value / r216_label (KBLI benar)
- r215c_url (opsional, tidak dipakai untuk anomali)
""")
...
label_map = {
 '10':'Industri Makanan','11':'Industri Minuman','12':'Industri Pengolahan Tembakau','13':'Industri Tekstil',
 '14':'Industri Pakaian Jadi','15':'Industri Kulit & Alas Kaki','16':'Industri Kayu','17':'Industri Kertas',
 '18':'Industri Pencetakan','19':'Industri Batu Bara','20':'Industri Kimia','21':'Industri Farmasi',
 '22':'Industri Karet & Plastik','23':'Industri Barang Galian','24':'Industri Logam Dasar',
 '25':'Industri Barang Logam','26':'Industri Komputer','27':'Industri Listrik','28':'Industri Mesin',
 '29':'Industri Kendaraan','30':'Industri Alat Angkut','31':'Industri Furnitur','32':'Industri Pengolahan Lainnya',
 '33':'Reparasi Mesin'
}

# ---------- CLEAN ITERATIVE RULES (TANPA REGEX ERROR) ----------
def apply_iterative_rules_fixed(df):

    df = df.copy()
    txt = df["text"].str.upper()

    # RULES EKSEPSI (harus ditangkap dahulu)
    exception_rules = [
        (lambda t: ("SABLON" in t and ("KAOS" in t or "KEMEJA" in t or "BAJU" in t)), "14"),
        (lambda t: ("REBANA" in t or "GENDANG" in t or "ALAT MUSIK" in t), "32"),
        (lambda t: ("BATU BATA" in t or "BATAKO" in t or "BATACO" in t), "23"),  # masuk C-23
    ]

    # RULE UTAMA (aman tanpa look-behind)
    base_rules = [
        (lambda t: ("CETAK" in t or "PERCETAKAN" in t or "UNDANGAN" in t)
                   and ("KAOS" not in t and "KEMEJA" not in t and "BAJU" not in t), "18"),
        (lambda t: ("KERTAS" in t), "17"),
        (lambda t: ("KARET" in t or "PLASTIK" in t), "22"),
        (lambda t: ("LEMARI" in t or "KURSI" in t or "MEJA" in t), "31"),
        (lambda t: ("MESIN" in t or "DINAMO" in t or "POMPA" in t or "KOMPRESOR" in t), "28"),
        (lambda t: ("TEPUNG" in t or "KUE" in t or "TAHU" in t or "TEMPE" in t), "10"),
    ]
    ...
    for i, row in df.iterrows():
        ...
        # Eksepsi dulu
        for cond, tgt in exception_rules:
            if cond(t):
                df.at[i, "kbli2_pred"] = tgt
                df.at[i, "kbli2_pred_label"] = label_map[tgt]
                break

        # Kalau belum berubah
        if df.at[i, "kbli2_pred"] not in label_map:
            for cond, tgt in base_rules:
                if cond(t):
                    df.at[i, "kbli2_pred"] = tgt
                    df.at[i, "kbli2_pred_label"] = label_map[tgt]
                    break
...
if uploaded_file is not None:
    ...
    if raw_name.endswith(".xlsx") or raw_name.endswith(".xls"):
        df = pd.read_excel(uploaded_file)
    else:
        raw_bytes = uploaded_file.getvalue()
        enc = (chardet.detect(raw_bytes)['encoding'] or 'utf-8')
        df = pd.read_csv(StringIO(raw_bytes.decode(enc, errors='replace')))

    df.columns = [c.strip() for c in df.columns]

    # Build text
    feat_cols = [c for c in ["r215a1_label", "r215b", "r215d"] if c in df.columns]
    df["text"] = df[feat_cols].fillna("").agg(" ".join, axis=1)
    ...
    # ---------- Model TF-IDF ----------
    st.subheader("Training Model TF-IDF (lebih akurat)")
    labeled = df["kbli2_true"].notna().sum()

    if labeled < 20:
        st.error("Label r216 terlalu sedikit untuk melatih model.")
        st.stop()
    ...
    pred = tfidf_model.predict(X)
    proba = tfidf_model.predict_proba(X).max(axis=1)

    df["kbli2_pred"] = pred
    df["kbli2_pred_label"] = df["kbli2_pred"].map(label_map)
    df["kbli2_pred_proba"] = proba

    # ---------- Iterative Rule Fixed ----------
    df = apply_iterative_rules_fixed(df)
    ...
    df["status_kesesuaian"] = np.where(
        ~df["is_catC_pred"] & df["is_catC_true"], "True C VS Pred non-C",
        np.where(df["is_catC_pred"] & ~df["is_catC_true"], "True non-C VS Pred C",
        np.where(mismatch, "Mismatch C", "Sesuai C"))
    )
    ...
    # Download
    st.download_button("Download Hasil (CSV)", df.to_csv(index=False).encode("utf-8"), "hasil_kbli.csv")
