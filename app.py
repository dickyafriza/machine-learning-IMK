...
st.set_page_config(page_title="VIMK25 Anomaly Detector", layout="wide")

st.title("🔍 IMK Anomaly Detection System")
st.subheader("Berdasarkan Pedoman Pemeriksaan VIMK25-L2")

uploaded_file = st.file_uploader("Unggah File CSV Hasil Pencacahan", type=["csv"])

if uploaded_file:
    df = pd.read_csv(uploaded_file)
    
    # --- Logic Pemeriksaan sesuai PPT ---
    def check_logic(row):
        errors = []
        if row['r304_value'] in [1, 2] and pd.isna(row['r314']):
            errors.append("Nama Usaha (314) Kosong")
        if not pd.isna(row['r316b']) and pd.isna(row['r317_label']):
            errors.append("KBLI (317) Kosong tapi 316 Terisi")
        return ", ".join(errors) if errors else "Clear"

    df['Logical_Check'] = df.apply(check_logic, axis=1)

    # --- Machine Learning Detection ---
    features = ['r304_value', 'r308_value', 'r310_value', 'r311_value', 'r318']
    ...
    df['ML_Anomaly'] = clf.fit_predict(X)
    df['Status'] = df['ML_Anomaly'].apply(lambda x: "Perlu Cek" if x == -1 else "Aman")
    ...
    with col2:
        st.metric("Potensi Anomali (ML)", len(df[df['Status'] == "Perlu Cek"]))

    st.write("### Daftar Data Terindikasi Anomali")
    display_cols = ['id', 'nama', 'r304_value', 'r314', 'Logical_Check', 'Status']
    st.dataframe(df[df['Status'] == "Perlu Cek"][display_cols], use_container_width=True)

    # Plot
    fig = px.histogram(df, x="Status", color="Status", title="Distribusi Status Validitas Data")
    st.plotly_chart(fig)