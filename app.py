...
st.caption(
    "Upload CSV (boleh kotor), pembersihan ringan, klasifikasi 2 digit (Kategori C), "
    "koreksi aturan, dan unduh hasil."
)
...
def detect_and_read_csv(file_bytes: bytes) -> pd.DataFrame:
    ...
    # Buang header catatan di atas data
    while lines and (
...
def apply_iterative_rules_simple(
    df: pd.DataFrame,
    cols,
    max_iters: int = 3,
    conf_thr: float = 0.80,
    ...
    rules = [
        ...
        (
            r'\b(TEPUNG|BERAS|KUE|ROTI|BOLU|BISKUIT|KERIPIK|ABON|DODOL|GETHUK|TAHU|TEMPE|TAPAI|SAMBAL|SAUS|KECAP|MIE|SELAI|SIRUP|ES|MANISAN|BUBUR|BROWNIES|RENDANG|SERUNDENG)\b',
            '10',
        ),
        ...
        (
            r'\b(KARET|PLASTIK)\b.*\b(PIPA|SELANG|MOLD|MOLDING|INJECTION|PACKAGING|PE|PP|PVC|LISTRIK|SEAL)\b',
            '22',
        ),
    ]
    ...
    while changed and it < max_iters:
        ...
        for pattern, target in rules:
            m = (
                cand
                & txt.str.contains(pattern, regex=True, na=False)
                & (out2['kbli2_pred'] != target)
            )
            if m.any():
                out2.loc[m, 'kbli2_pred'] = target
                out2.loc[m, 'kbli2_pred_label'] = out2.loc[
                    m, 'kbli2_pred'
                ].map(label_map)
                changed = True
...
def build_or_load_pipeline(feature_names):
    ...
    # Kalau tidak ada, buat pipeline baru
...
st.sidebar.markdown(
    "- Pastikan kolom r215a1_label / r215b / r215d ada di CSV.\n"
    "- Opsional: r213, r216_value / r216_label untuk label asli."
)

uploaded = st.file_uploader(
    "Upload CSV (boleh kotor/bersih)", type=["csv"]
)
...
if uploaded and proses:
    ...
    st.success(f"File terbaca dengan {df.shape[0]} baris dan {df.shape[1]} kolom.")
    ...
    has_y = (
        df['kbli2_true'].notna().sum() >= 50
        and df['kbli2_true'].nunique() >= 2
    )
    ...
    # Aturan iteratif
    out_iter = apply_iterative_rules_simple(
        out, feat_cols, max_iters=3, conf_thr=0.80
    )
    ...
    st.markdown("**Klasifikasi (semua baris)**")
    st.dataframe(klasifikasi.head(20))

    st.markdown("**Bersih (Sesuai C)**")
    st.dataframe(bersih.head(20))

    st.markdown("**Anomali / Perlu Cek**")
    st.dataframe(anomali.head(20))
